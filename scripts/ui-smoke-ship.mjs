...
const stat = async (label) => {
  // Budget figures are label/value rows now; labels are uppercase in the copy.
  const el = page.locator(".budget-panel .frow", { has: page.locator(".flabel", { hasText: new RegExp(`^${label}`, "i") }) }).first();
  return (await el.count()) ? (await el.locator(".val").innerText()).trim() : "";
};
...
const warnCells = await page.locator("table.modes td.tone-violation").count();
...
const chips = await page.locator(".budget-panel .group", { hasText: "SECTIONS" }).locator(".frow").allInnerTexts();
if (!chips.some((c) => /m³/.test(c))) fail("no section volume rows");
console.log("sections:", chips.filter((c) => /m³/.test(c)).join(" · "));

const advisories = await page.locator(".budget-panel .adv-list .srow").count();
...
const clean = await page.locator(".budget-panel .adv-list .srow").allInnerTexts();
